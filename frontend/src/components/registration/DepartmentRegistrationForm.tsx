// DepartmentRegistrationForm.tsx
...
export default function DepartmentRegistrationForm({ onBack, onSuccess }: Props) {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (!form.tpo_profile_id.trim()) {
      setError("TPO profile ID is required.");
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="mb-6">
        ...
        <h2 className="mt-3 text-lg font-serif font-medium text-[#2b2318]">Department Registration</h2>
        <p className="mt-1 text-sm text-[#8a7a5c]">Your account will require TPO approval.</p>
      </div>
      ...
      <label className={labelCls}>
        Officer Email
        <input className={inputCls} type="email" name="officer_email" value={form.officer_email} onChange={handleChange} required />
      </label>

      <label className={labelCls}>
        Officer Contact Number
        <input className={inputCls} name="officer_contact" value={form.officer_contact} onChange={handleChange} required />
      </label>

      <label className={labelCls}>
        Department Name
        <input className={inputCls} name="department_name" value={form.department_name} onChange={handleChange} required />
      </label>
